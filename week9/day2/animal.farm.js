...
var farm = {
    ...
    breed: function () {
        ...
        if (this.animalList.length < this.places) {
            this.animalList.push(a);
        };
    },
    slaughter: function () {
        var hungerList = [];
        this.animalList.forEach(function(element) {
            hungerList.push(element.hunger);
        });
        for (var i = 0; i < this.animalList.length; i++) {
            if (this.animalList[i].hunger === Math.min(...hungerList)) {
                var leastHungry = i;
            };
        };
        this.animalList.splice(leastHungry, 1);
    },
};
...
farm.breed()
farm.breed()
farm.animalList[2].eat();
farm.animalList[0].eat();
farm.animalList[1].eat();
farm.animalList[1].eat();
farm.slaughter()
farm.slaughter()

//
// console.log(farm.animalList[0].hunger);
